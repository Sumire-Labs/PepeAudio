...
public static class AuraConvolution
{
    ...
    public static string Build(HeSuViPreset preset, string pre, string outFormat)
    {
        ...
        return preset.Channels switch
        {
            ...
            4 => $"{pre},pan=4C|c0=FL|c1=FL|c2=FR|c3=FR[a];amovie={ir}[ir];" +
                 $"[a][ir]{Afir}[b];[b]pan=stereo|FL<c0+c2|FR<c1+c3,{makeup},{outFormat}[out]",
            2 => $"{pre}[a];amovie={ir}[ir];[a][ir]{Afir},{makeup},{outFormat}[out]",
            _ => $"{pre},{outFormat}[out]",
        };
    }
    ...
    private static string MakeupTail(double db)
        => $"volume={db.ToString("0.##", CultureInfo.InvariantCulture)}dB,alimiter=limit=0.95";
    ...
    private static string Immersive(string ir, string pre, string makeup, string outFormat)
    {
        return
            ...
            $"amovie={ir},asplit=4[i0][i1][i2][i3];" +
    ...
    // amovie filename sits inside a filter option, so ':' (a Windows drive letter) needs the
    // two-level filtergraph escape '\\:' — a single backslash is consumed by the graph parser.
