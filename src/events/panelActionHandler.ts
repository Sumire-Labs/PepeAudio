...
export async function handleButtonOrSelect(interaction: ButtonInteraction | StringSelectMenuInteraction): Promise<void> {
  ...
  if (!player || player.destroyed) {
    await safeReply(interaction, { content: 'このパネルは無効です。`/now` で再表示してください。', flags: MessageFlags.Ephemeral });
    return;
  }
  ...
  if (!perm.ok) {
    await safeReply(interaction, { content: perm.reason ?? '権限がありません。', flags: MessageFlags.Ephemeral });
    return;
  }
  ...
  try {
    ...
    await safeReply(interaction, { content: '操作に失敗しました。もう一度お試しください。', flags: MessageFlags.Ephemeral }).catch(
      (followUpErr) => logger.debug({ err: followUpErr }, 'Failed to send panel action failure notice'),
    );
  }
}
