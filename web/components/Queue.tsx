...
import type { QueueItem, SearchCandidate } from "@/lib/types";
import { api, UnauthorizedError } from "@/lib/api";
import { formatMs } from "@/lib/format";
import { cx, Icons, Spinner, type Icon } from "@/components/ui";
...
export function Queue({ session, onSaveTrack }: { session: PlayerSession; onSaveTrack: (t: QueueItem) => void }) {
  ...
  // cmd.play rejects when the user isn't in a voice channel / on backend errors.
  ...
  const requeue = async (url: string) => {
    try {
      await cmd.play(url);
      toast("キューに追加しました。");
    } catch (err) {
      ...
      <form onSubmit={submitAdd} className="glass mb-2 flex items-center gap-2 rounded-2xl px-3.5 py-2.5">
        ...
        {query ? (
          <button
            ...
            className="grid h-6 w-6 flex-none place-items-center rounded-full text-[var(--text-faint)] hover:text-[var(--text)]"
          >
      ...
      {candidates ? (
        <div className="glass mb-2 max-h-72 overflow-y-auto soft-scroll rounded-2xl p-1.5 fade-in">
          <div className="flex items-center justify-between px-2 py-1">
            ...
            <button onClick={() => setCandidates(null)} className="text-xs text-[var(--text-faint)] hover:accent">
              閉じる
            </button>
          </div>
          {candidates.map((c) => (
      ...
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto soft-scroll pr-1">
        <section>
          <div className="mb-2 flex items-center justify-between px-1">
            ...
            {queue.length > 0 ? (
              <button onClick={() => cmd.clearQueue()} className="text-xs text-[var(--text-faint)] transition hover:accent">
                すべてクリア
              </button>
            ) : null}
          </div>
            ...
            <ul className="space-y-1">
              {queue.map((track, i) => (
                <TrackRow
                  ...
                  onDropHere={() => onDrop(i)}
                  onJump={() => cmd.jumpTo(track.id)}
                  onRemove={() => cmd.removeTrack(track.id)}
            ...
            <ul className="space-y-1 opacity-70">
              {[...history].reverse().map((track) => (
                <TrackRow
                  key={`${track.id}-h`}
                  track={track}
                  onRequeue={() => void requeue(track.sourceUrl)}
                  onSave={() => onSaveTrack(track)}
                />
              ))}
            </ul>
...
function RowBtn({ icon: IconComp, label, onClick }: { icon: Icon; label: string; onClick: () => void }) {
  return (
    <button
      type="button"
      aria-label={label}
