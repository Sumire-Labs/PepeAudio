...
export function Queue({ session, onSaveTrack }: { session: GuildSession; onSaveTrack: (t: QueueItemDTO) => void }) {
  ...
  return (
    <div className="flex h-full flex-col">
      <form onSubmit={submitAdd} className="glass mb-3 flex items-center gap-2 rounded-2xl px-3.5 py-2.5">
        ...
        <button
          ...
          aria-label="追加"
        >
          {adding ? <Spinner className="h-4 w-4" /> : <Icons.Plus className="h-4 w-4" />}
        </button>
      </form>
...
function TrackRow({
  ...
  return (
    ...
    >
      {draggable ? <Icons.Grip className="h-4 w-4 flex-none text-[var(--text-faint)] opacity-0 transition group-hover:opacity-100" /> : index ? null : null}
      {track.thumbnailUrl ? (
